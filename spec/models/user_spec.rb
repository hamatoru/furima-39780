...
describe 'ユーザー新規登録できるとき' do
  ...
  describe 'ユーザー新規登録できないとき' do
    ...
    it "last_nameにカナ以外が含まれていると登録ができない" do
      @user.last_name_kana = '田ナカ'
      @user.valid?
      expect(@user.errors.full_messages).to include "Last name kana は全角カタカナ"
    end
    it "first_nameにカナ以外が含まれていると登録ができない" do
      @user.first_name_kana = 'タろう'
      @user.valid?
      expect(@user.errors.full_messages).to include "First name kana は全角カタカナ"
    end
